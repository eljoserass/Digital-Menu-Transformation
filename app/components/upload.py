...
def _selected_file_view(file: str) -> rx.Component:
    ...
    return rx.el.div(
        rx.icon("file-image", class_name="h-5 w-5 text-gray-400"),
        rx.el.p(file, class_name="text-sm text-gray-300 truncate"),
        class_name="flex items-center gap-2 p-2 bg-gray-800 border border-gray-700 rounded-lg w-full",
    )
...
def _qr_code_display() -> rx.Component:
    ...
    return rx.el.div(
        rx.el.h3(
            "Your menu is ready!",
            class_name="text-xl font-bold text-center text-gray-100",
        ),
        rx.el.p(
            "Scan the QR code to view your menu.",
            class_name="text-center text-gray-400 mt-2 mb-4",
        ),
        rx.image(
            ...
            width=200,
            height=200,
            class_name="mx-auto border-4 border-gray-800 rounded-lg shadow-lg",
        ),
        rx.el.input(
            default_value=UploadState.menu_url,
            read_only=True,
            class_name="w-full mt-4 p-2 text-center bg-gray-700 border border-gray-600 rounded-md text-sm text-gray-300",
        ),
        rx.el.a(
            "Open Menu",
        ...
        rx.el.button(
            ...
            class_name="mt-2 w-full text-center bg-gray-600 text-gray-200 font-medium py-2 px-4 rounded-lg hover:bg-gray-500 transition-colors",
        ),
        class_name="bg-gray-900 p-8 rounded-xl shadow-lg border border-gray-700 w-full max-w-sm",
    )
...
def upload_page() -> rx.Component:
    ...
    return rx.el.div(
        rx.cond(
            ...
            rx.el.div(
                rx.el.h2(
                    "Upload Your Menu", class_name="text-2xl font-bold text-gray-100"
                ),
                rx.el.p(
                ...
                rx.upload.root(
                    rx.el.div(
                        rx.icon(
                            "cloud-upload", class_name="w-12 h-12 stroke-gray-500 mb-4"
                        ),
                        rx.el.p(
                            rx.el.strong("Click to upload", class_name="text-red-500"),
                            " or drag and drop",
                        ...
                        class_name="flex flex-col items-center justify-center p-8 border-2 border-dashed border-gray-600 rounded-lg bg-gray-800 text-center",
                    ),
                    id=UPLOAD_ID,
                ...
                rx.cond(
                    ...
                    rx.el.div(
                        ...
                        UploadState.error_message,
                        class_name="flex items-center mt-4 text-sm text-red-500 bg-red-900 p-3 rounded-lg",
                    ),
                    None,
                ),
                _upload_button(),
                rx.el.a(
                    "< Back to Menu",
                    href="/menu/sample",
                    class_name="mt-6 text-sm text-red-500 hover:underline text-center block",
                ),
                class_name="bg-gray-900 p-8 rounded-xl shadow-lg border border-gray-700 w-full max-w-lg",
            ),
        ),
        class_name="flex items-center justify-center min-h-screen bg-black p-4",
    )